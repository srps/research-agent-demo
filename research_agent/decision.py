...
class DecisionAgent:
	...
	def is_research_complete(self, research_summaries: list, topic: str, api_key: str) -> ResearchDecision:
		...
		try:
					...
					text={
						'format': {
							...
							'schema': {
								...
								'required': ['is_complete', 'reasoning', 'gaps'],
			...
			self.logger.debug(
				f'Model used: {response.model if hasattr(response, "model") else "N/A"}'
			)
			self.logger.debug(
			...
			try:
				...
				self.logger.info(
					f'Decision: {decision.is_complete}, Reasoning: {decision.reasoning[:100]}...'
				)
				return decision
