...
class TriageDecision(BaseModel):
    ...
    clarification_question: Optional[str] = None  # Question to ask the user if clarification is needed
...
class TriageAgent:
    ...
    def triage_query(self, query: str, api_key: str, conversation_history: Optional[list] = None) -> TriageDecision:
        ...
        try:
            ...
            if len(self.conversation_history) > 1:  # If there's more than just the current query
                for i, message in enumerate(self.conversation_history[:-1]):  # Exclude the current query
                          ...
                          "schema": {
                            ...
                            "properties": {
                              "status": {
                                ...
                                "description": "The status of the user query"
                              },
                              "reasoning": {
                                "type": "string",
                                "description": "Explanation of why the query has this status"
                              },
            ...
            try:
                ...
                self.logger.info(f"Triage Decision: {decision.status}, Reasoning: {decision.reasoning[:100]}...")
                return decision
